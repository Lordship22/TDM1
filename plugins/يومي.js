...
const handler = async (m, {isPrems, conn}) => {
  ...
  if (new Date - user.lastclaim < 7200000) return await conn.reply(m.chat, `لقد طالبة بالفعل بهديتك اليومية 🎁\n عد بعد _*${msToTime(time - new Date())}*_ \n للمطالبة مره اخرى`, fkontak, m);
  ...
  const text = `╭━━🎁━🎁━🎁━━⬣
┃l *✨ عااااااش المكافاءة اليومية !!✨*
┃l ${premium ? '🎟️ مكافأة مميزة' : '🆓 lمكافأة مجانية'}
┃l ${texto}
...
handler.help = ['daily'];
handler.tags = ['xp'];
handler.command = ['daily', 'يومي','reclamar', 'reclamo', 'regalo', 'claim'];
